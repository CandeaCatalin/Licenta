...
export const AddQueueModal: FC<AddQueueModalProps> = ({onClose}) => {
    ...
    const addNewPhysicalQueue = () => {
        setPhysicalQueues([...physicalQueues, {
            name: "",
            description: "",
            id: 0,
            queueId: addQueue.id,
    ...
    const updatePhysicalQueueDescription = (index: number, newDescription: string) => {
        const list = physicalQueues;
        list.map((item, i) => {
            if (i === index) {
                    ...
                    <div className="modal-body" style={{maxHeight: "50vh", overflow: "auto"}}>
                        <div className="form-floating mb-3">
                            <input
                                ...
                                onChange={(e) => setAddQueue({...addQueue, name: e.target.value})}
                        ...
                        <div className="form-floating mb-3">
                            <textarea
                                ...
                                maxLength={200}
                                cols={100}
                                onChange={(e) => setAddQueue({...addQueue, description: e.target.value})}
                            />
                        ...
                        {physicalQueues.map((item, index) => {
                            return <AddPhysicalQueue key={index}
                                                     onNameChange={(index: number, text: string) => updatePhysicalQueueName(index, text)}
                                                     onDescriptionChange={(index: number, text: string) => updatePhysicalQueueDescription(index, text)}
                                                     index={index}/>;
                        })}

                        <button onClick={() => addNewPhysicalQueue()}
                                className="btn button-modal-prim" style={{marginTop: "20px"}}><span
                            style={{fontWeight: "bold"}}>Add new location</span></button>
                    </div>
